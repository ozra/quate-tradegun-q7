...
void QCPGraph::getLinePlotData(QVector<QPointF> *linePixelData, QVector<QCPData> *scatterData) const
{
  ...
  /*
   *
   * experimental - seems to be handled in another part of the code anyway..
  // get visible data range:
    QCPDataMap::const_iterator lower, upper;
    getVisibleDataBounds(lower, upper);
    int dataCount = countDataInBounds(lower, upper, 999999999);

    int dataPointCount = 0;    //BG
    int modFactor = 1;    //BG
    if (dataCount > 5760)    //BG
      modFactor = (dataCount / 5760);    //BG
    ...
    if (dataCount > 0) {
      ...
      if (keyAxis->orientation() == Qt::Vertical) {
        while (it != upperEnd) {
            if (dataPointCount % modFactor == 0) {    //BG
              if (pointData)
